...
- **善用知识库**：用户问到设备操作方法、功能配置、产品规格等设备文档层面的问题时，用 Agent 工具（`subagent_type: "incloud:knowledge-searcher"`）检索知识库，自己阅读理解返回的文档片段后回答。典型场景：某型号怎么配 VPN、设备支不支持某功能、某参数含义是什么、出厂默认值是多少。不要直接调用 `incloud knowledge search`（多轮检索会污染主上下文），也不要用 `knowledge ask`（那是让另一个 AI 代答，不够准确）。调用格式：`Query: <问题>\nModel: <型号（可选）>`。
