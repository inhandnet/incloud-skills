...
description: >-
  ...
  "查看订单", "升级 License 类型", "查知识库", "设备文档",
  "设置 POS 优先级", "查看 POS Ready 状态", "POS 客户端", "POS 自定义规则",
  "查看 POS 命中", "POS 流量优先",
  or any device management, network diagnostics, knowledge base queries,
  and platform administration tasks via incloud CLI.
version: 0.5.0
---
...
调用 `incloud` 命令时，**必须**通过环境变量 `INCLOUD_CLIENT=claude-skill` 标识调用来源，让平台服务端能区分「AI 技能调用」与「人工操作」（用于用量统计）。

由于每条命令在独立 shell 中执行，环境变量不跨命令保留，因此**每一次** `incloud` 调用都要内联该前缀：

```bash
INCLOUD_CLIENT=claude-skill incloud device list
INCLOUD_CLIENT=claude-skill incloud alert list --after 2026-01-01T00:00:00
```

该值会被 CLI 追加到 User-Agent（形如 `incloud-cli/<版本> (<os>/<arch>) claude-skill`）。规则：
- 值固定为 `claude-skill`，不带版本号——服务端按 `claude-skill` 匹配 AI 流量。CLI/skill 版本无需在此维护，避免发版时漂移。
...
> 以下示例为简洁省略了 `INCLOUD_CLIENT=claude-skill` 前缀。**实际执行时，每条 `incloud` 命令都必须按「调用约定」一节带上该前缀**（示例仅展示子命令与参数形态）。
...
incloud device client mark-asset                  # 终端标记为网络资产
incloud device client set-pos-ready <client-id> --level priority|default|bypass  # 设置终端 POS 优先级
...
incloud device log local/syslog/mqtt/diagnostic    # 日志
...
> POS 优先级三态：`priority`（优先转发该终端 POS 流量）、`default`（不特殊处理，等价未标记）、`bypass`（排除在 POS 处理之外）。设置入口在 `device client set-pos-ready`，查询与观测在 `pos` 组。

```
incloud pos clients [--level priority|bypass] [--device <id>] [--oid <id>]  # 已标记 POS 优先级的终端
incloud pos marked-clients <device-id> [--level <l>]  # 某设备上已标记的终端
incloud pos forwarded [--active-within 24h] [--vendor <v>] [--client-type <t>]  # 近期命中转发的 POS 终端
incloud pos device-hits <device-id> [--active-within 24h] [--group-by vendor|client]  # 某设备的 POS 命中统计
incloud pos vendor-hits <device-id> <client-id> --after --before [--interval 5m]  # 终端按 vendor 的命中时序（时间必填）
incloud pos vendor-summary <device-id> <client-id> --after --before  # 终端按 vendor 的命中汇总（时间必填）
incloud pos client-types                          # 终端类型字典
incloud pos rules get <device-id>                 # 查设备的 POS 自定义规则
incloud pos rules set <device-id> --file <json|->  # 下发 POS 自定义规则（- 读 stdin）
incloud pos rules list [--device <id>]            # 列出所有设备的 POS 自定义规则
```

### 知识库

```
incloud knowledge search <query> [--model] [--rewrite] [--limit]  # 检索设备文档（AI agent 首选）
incloud knowledge ask <question> [--model]                        # AI 问答（面向人类用户）
```
...
**输入**：`--after/--before` 接受本地时间（如 `2026-03-27T16:39:44`、`2026-03-27`），CLI 会自动转为 UTC 发送给 API。也接受显式 UTC（带 `Z`）或带偏移量的格式。

**输出**：
- **table 模式**（默认）：时间已转为本地时间，直接展示，无需换算
- **json/jq 模式**：时间是 API 原始值（通常 UTC，带 `Z`），展示给用户前用 `date +%z` 获取系统时区偏移量进行换算
...
| 查日志/看 syslog/设备本地日志/下载诊断日志 | `references/log-analysis.md`       |
...
| POS 优先级/POS Ready/POS 命中/自定义规则   | `references/pos-management.md`     |
