...
- **远程访问**：Web UI / CLI 隧道，远程连接设备管理界面或终端
- **配置管理**：查看/更新配置、快照与回滚、配置复制
- **告警处理**：查看/确认/导出告警、告警规则 CRUD、Webhook 通知
- **固件升级**：固件查询、OTA 任务创建与跟踪
- **网络服务**：SD-WAN、InCloud Connector、OOBM 带外管理
- **资产管理**：网络资产 CRUD、客户端标记为资产
- **平台管理**：用户/角色/组织、活动日志、平台概览与趋势
...
6. **跨资源引用**：需要 ID 时先用对应 list 命令查询（如 `device group list`、`firmware list`、`role list`）。用户提供 SN 时，必须先通过 `incloud device list -q <SN> -f _id,serialNumber` 查出 `_id` 再操作（字段含义见上方"常见 ID 字段辨析"）。
