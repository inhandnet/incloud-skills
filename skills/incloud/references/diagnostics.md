...
incloud device log syslog <id>             # 7. 设备系统日志（网络/VPN/WiFi/DNS 等模块）
```
...
- 蜂窝设备检查信号质量，`antenna` 看各天线信号历史，`signal list` 看整体蜂窝信号趋势，信号弱或持续劣化可能导致频繁掉线
...
- 速度慢：对比历史速度测试，确认是否劣化
- 以上手段未定位原因时，查日志寻找具体错误信息：

```bash
incloud device log syslog <id>             # 设备系统日志（网络/VPN/WiFi/DNS 等模块），按关键词过滤
