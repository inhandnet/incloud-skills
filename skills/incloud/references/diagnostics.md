...
incloud device antenna <id>               # 5. 天线信号（蜂窝设备），信号是否过弱
incloud device perf <id>                   # 6. CPU/内存/磁盘，是否资源耗尽
incloud device log syslog <id>             # 7. 系统日志，查找错误和异常
```
...
incloud device exec capture <id> --interface <iface>  # 启动抓包
incloud device exec capture-status <id>               # 查看抓包状态
