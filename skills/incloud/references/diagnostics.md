...
incloud device signal list <id>            # 5. 蜂窝信号历史（整体信号质量趋势）
incloud device antenna <id>               # 5b. 天线信号历史（按天线维度的 RSRP/RSRQ/SINR）
...
排查思路：
- 先定位问题出在哪一侧：平台侧（API/连接状态）还是设备侧（本地行为）。平台侧数据（在线状态、信号、性能）是设备上报的快照，能定位现象；设备侧根因需要看 syslog 中设备本地的实际行为
- 先确认设备是否真的离线（可能只是平台连接断开）
- 查看最后在线时间，判断是突然断开还是逐渐失联。`disconnectReason` 是 AWS IoT Core（broker 侧）记录的云端视角断连原因，反映的是 broker 的观测结果，不直接等同于设备或网络侧的根因
- **先定时间窗口再查数据**：根据已知线索（用户描述、告警时间、在线历史等）确定关注的时间段，查时序数据时用 `--after/--before` 聚焦，不要拉全量
- 检查网络接口和上行链路，确认物理连接状态。uplink 的延迟、吞吐量等指标是设备上报的遥测快照，反映链路表现；当表现异常但原因不明时，需结合 syslog 进一步定位
- 蜂窝设备检查信号质量，`signal list` 看整体信号趋势（首选），`antenna` 看各天线维度的信号历史（补充），信号弱或持续劣化可能导致频繁掉线。**判读信号数值时加载 `references/signal-analysis.md`**
- 检查资源使用率，CPU 或内存打满可能导致服务异常
- 查看日志寻找具体错误信息。**分析日志内容时加载 `references/log-analysis.md`** 获取模块分类和关键词映射
- 以上手段仍不足时，可通过隧道登录设备 CLI 查看 INOS 配置、路由表等平台未暴露的信息。**操作前加载 `references/device-cli-tunnel.md`**
- 设备已离线、远程手段全部不可用时，引导用户到设备现场，收集本地文件后发给你分析（见下方"离线诊断替代路径"）

### 离线诊断替代路径

设备离线后无法执行任何远程操作。引导用户到现场，从设备 Web UI 导出以下文件后发给你分析：

- **syslog**：掉线前后的错误事件和模块行为
- **诊断包**：接口、路由、进程、内核日志的综合快照
- **配置文件**：排查配置错误或与预期不符的设置
